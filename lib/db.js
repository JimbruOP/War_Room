...
import { startOfQuotaDay } from "./schedule";
...
export async function fetchFeedStatus(supabase) {
  ...
  const [lastRes, todayRes] = await Promise.all([
    supabase
      .from("fetch_log")
      .select("created_at, tier, fetched, inserted, triggered_by")
      .order("created_at", { ascending: false })
      .limit(1)
      .maybeSingle(),
    supabase
  ...
  const creditsUsed = (todayRes.data || []).reduce(
    (sum, r) => sum + (r.queries_used || 0),
    0
  );

  return { last: lastRes.data || null, creditsUsed };
}
