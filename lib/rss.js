...
// Malayalam equivalent, for Malayalam-script feeds that carry a general news
// mix. Covers: chief minister, government, minister, police, court, protest,
// strike, election, arrest, controversy, party names, Kerala.
const MALAYALAM_NEWS_RE =
  /(മുഖ്യമന്ത്രി|സർക്കാർ|മന്ത്രി|പോലീസ്|കോടതി|സമരം|പ്രതിഷേധ|തിരഞ്ഞെടുപ്പ്|അറസ്റ്റ്|വിവാദ|ബിജെപി|കോൺഗ്രസ്|സിപിഎം|സിപിഐ|യുഡിഎഫ്|എൽഡിഎഫ്|മുസ്ലിം ലീഗ്|കേരള|വിദ്യാഭ്യാസ|സഭ)/;
...
export const FEEDS = [
  ...
  // Mathrubhumi. Their own RSS endpoints are gone (every /rss and /feed path
  // 404s, the old cmlink IDs return 410, and the FeedBurner feed is abandoned —
  // 50 items, none newer than 48h). Reaching them via a Google News site: query
  // is the only route that still returns live articles.
  // Filtered because Mathrubhumi's general feed mixes tech and lifestyle in
  // with the politics.
  {
    id: "mathrubhumi-ml",
    cat: "kerala",
    source: "Mathrubhumi",
    url: "https://news.google.com/rss/search?q=site:mathrubhumi.com&hl=ml&gl=IN&ceid=IN:ml",
    include: MALAYALAM_NEWS_RE,
  },
  {
    id: "mathrubhumi-en",
    cat: "kerala",
    source: "Mathrubhumi English",
    url: "https://news.google.com/rss/search?q=site:english.mathrubhumi.com&hl=en-IN&gl=IN&ceid=IN:en",
    include: POLITICS_RE,
  },
...
export async function fetchAllFeeds() {
  ...
  // Dedupe on URL *and* on normalised headline. URL alone is not enough:
  // Google News hands out a different redirect URL for the same article
  // depending on which query surfaced it, so one story arriving via two feeds
  // would otherwise appear twice.
  const seenUrl = new Set();
  const seenTitle = new Set();
  const deduped = stories.filter((s) => {
    const titleKey = s.headline
      .toLowerCase()
      .replace(/[^\p{L}\p{N}]+/gu, "") // strip punctuation/space, keep any script
      .slice(0, 120);
    if (seenUrl.has(s.url_hash) || seenTitle.has(titleKey)) return false;
    seenUrl.add(s.url_hash);
