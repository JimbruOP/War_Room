...
// Manual "Refresh now". Defaults to the core (Kerala) tier — the cheapest
// useful fetch. The signed-in session is what authorises this server-side.
export async function refreshNews(tier = "core") {
  const res = await fetch(`/api/news/refresh?tier=${tier}`, { cache: "no-store" });
