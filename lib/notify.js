...
export async function sendHighPriorityAlerts(stories, siteUrl) {
  ...
  // Malayalam headlines are hard for the team to skim, so append a literal
  // English translation as a subtext line when the triage pass produced one.
  const withTranslation = (s) =>
    s.headline_en ? `${s.headline}\n\nTranslation: ${s.headline_en}` : s.headline;

  let heading, content, url;
  if (stories.length === 1) {
    heading = "🔴 High-priority story";
    content = withTranslation(stories[0]);
  ...
  const res = await fetch(ONESIGNAL_ENDPOINT, {
    ...
    body: JSON.stringify({
      app_id: appId,
      // OneSignal's current default segment. The older "Subscribed Users"
      // name returns "not subscribed" against the newer user model.
