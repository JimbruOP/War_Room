...
function buildPrompt(lens, headlines) {
...
async function scoreBatch(lens, headlines) {
  const openai = getOpenAI();
  const res = await openai.chat.completions.create({
    model: TRIAGE_MODEL,
    max_tokens: 4000,
    messages: [{ role: "user", content: buildPrompt(lens, headlines) }],
...
export async function triageStories(lens, stories) {
  const scored = [];
  const errors = [];
  let promptTokens = 0;
  ...
  for (let i = 0; i < batches.length; i += TRIAGE_CONCURRENCY) {
    ...
    const settled = await Promise.allSettled(
      wave.map((batch) => scoreBatch(lens, batch.map((s) => s.headline)))
    );
