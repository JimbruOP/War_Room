...
For "headline_en": if the headline is in Malayalam (or any non-English script), give a faithful, literal English translation of the headline as it is — just the title translated, not a summary or explanation. If the headline is already in English, return an empty string "".
  ...
  for (const row of parsed.scores || []) {
    if (row.i >= 0 && row.i < headlines.length) {
      out[row.i] = {
        ...
        headline_en: String(row.headline_en || "").trim().slice(0, 300) || null,
      };
    }
  }
